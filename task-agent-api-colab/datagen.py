...
config = {"root": "data"}  # Relative path
abs_path = os.path.abspath(config["root"])  # Calculate absolute path
...
def write_file(path, content):
    with open(os.path.join(abs_path, path), "w", encoding="utf-8") as f:
        f.write(content)
...
def a5_logs():
    ...
    email = config["email"]
    os.makedirs(os.path.join(abs_path, "logs"), exist_ok=True)
    now = time.time()
    for i, (age, text) in enumerate(get_logs(email)):
        write_file(f"logs/log-{i}.log", text)
        os.utime(os.path.join(abs_path, f"logs/log-{i}.log"), (now - age, now - age))
...
def a6_docs():
    ...
    email = config["email"]
    docs = get_docs(email)
    os.makedirs(os.path.join(abs_path, "docs"), exist_ok=True)
    for dir, file, text in docs:
        dirname = os.path.join(abs_path, "docs", dir)
...
def a8_credit_card_image():
    ...
    image.save(os.path.join(abs_path, "credit_card.png"))
...
def a10_ticket_sales():
    ...
    target = os.path.join(abs_path, "ticket-sales.db")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--root", default="data")  # Default is now relative
    args = parser.parse_args()
    config["email"] = args.email
    config["root"] = args.root  # Keep as relative path
    abs_path = os.path.abspath(config["root"]) #Create abs path from relative path


    os.makedirs(abs_path, exist_ok=True)

    print("DISCLAIMER: THIS SCRIPT WILL CHANGE BEFORE THE EVALUATION. TREAT THIS AS A GUIDE.")
    print("Files created at", abs_path)
